                   ...
                   Build_Data *build_data, String first_file_path)
{
    ...
    auto first_file_name =
    string_copy(allocator, file_name, file_name.length - 4);
    ...
    auto entry_file_path =
    string_append(allocator, resolver->module_dir, "entry.zdc");
    ...
    while (!done) {
        ...
        for (int64_t i = resolver->llvm_builder.struct_types_to_finalize.count - 1;
            ...
            if (to_finalize.ast_type->flags & AST_NODE_FLAG_RESOLVED_ID) {
                progressed = true;
                llvm_finalize_struct_type(&resolver->llvm_builder,
                                          to_finalize.llvm_type, to_finalize.ast_type);
        ...
        if (fatal_error_reported(resolver) ||
            (!progressed && !resolver->progressed)) {
                     ...
                     bool insert_entry_module /*= false*/)
{
    ...
    Parse_Job job = {
        .module_name = string_copy(resolver->allocator, module_name),
...
bool queue_parse_job(Resolver *resolver, String module_name,
                     String module_path)
{
    AST_Module *ex_module = nullptr;
    bool in_queue =
        queue_parse_job(resolver, module_name, module_path, &ex_module, false);
...
bool try_parse_job(Resolver *resolver, Parse_Job *job)
{
    ...
    if (job->insert_entry_module) {
        ...
        Lexed_File lf_entry =
            lexer_lex_file(&resolver->lexer, resolver->entry_module_path);
        ...
        Atom entry_module_name =
            atom_get(&resolver->build_data->atom_table, "entry");
    ...
    Token_Stream *token_stream =
        lexer_new_token_stream(resolver->allocator, &lexed_file);

    Parsed_File parsed_file = {};

    Atom module_name =
        atom_get(&resolver->build_data->atom_table, job->module_name);
    ...
    AST_Module *module_ast = ast_create_from_parsed_file(
    &resolver->ast_builder, &parsed_file, resolver->global_scope);
    //
    ...
    array_append(&resolver->parsed_modules,
                 {job->module_path, job->module_name, module_ast});
    ...
    for (int64_t i = flat_decl->flat->waiting_on; i < flat_decl->flat->nodes.count; i++) {
        ...
        switch (node->kind) {
            ...
            case AST_Node_Kind::IDENTIFIER:
                assert(false);
            case AST_Node_Kind::SWITCH_CASE:
                assert(false);
...
bool try_resolve_declaration(Resolver *resolver, AST_Declaration *declaration)
{
    switch (declaration->kind) {
        ...
        case AST_Declaration_Kind::IMPORT: {
            ...
            bool in_queue =
            queue_parse_job(resolver, module_name, file_path, &ast_module);
            if (in_queue)
                return false;
            ...
            while (scope_block) {
                for (int64_t i = 0; i < scope_block->decl_count; i++) {
                    ...
                    if (decl->identifier) {
                        ...
                        if (redecl) {
                            zodiac_report_error(
                                resolver->build_data, Zodiac_Error_Kind::REDECLARATION,
                    ...
                    if (ts->type != init_expr->type) {
                        if (is_valid_type_conversion(init_expr, ts->type)) {
                            ...
                            zodiac_report_error(resolver->build_data,
                                                Zodiac_Error_Kind::MISMATCHING_TYPES, init_expr,
                    ...
                    if (ts->type != init_expr->type) {
                        if (is_valid_type_conversion(init_expr, ts->type)) {
                            ...
                            zodiac_report_info(resolver->build_data, ts,
                                               "Expected type: '%.*s'", (int)exp_str.length,
                                               exp_str.data);
                            auto got_str = ast_type_to_tstring(init_expr->type);
                            zodiac_report_info(resolver->build_data, init_expr,
                                               "Given type: '%.*s'", (int)got_str.length,
...
#ifndef NDEBUG
            for (int64_t i = 0; i < declaration->function.parameter_declarations.count;
                 i++) {
                AST_Declaration *param_decl =
                declaration->function.parameter_declarations[i];
        ...
        case AST_Declaration_Kind::ENUM: {
            ...
            for (int64_t i = 0; i < declaration->enum_decl.member_declarations.count;
                 i++) {
            ...
            for (int64_t i = 0; i < declaration->enum_decl.member_declarations.count;
                ...
                if (mem_decl->decl_flags & AST_DECL_FLAG_ENUM_MEMBER_INTINIT) {
                    auto nv =
                    const_interpret_expression(mem_decl->constant.init_expression);
                    ...
                    assert(!mem_decl->constant.init_expression);
                    mem_decl->constant.init_expression = ast_integer_literal_expression_new(
                    resolver->allocator, next_value, member_scope,
                    mem_decl->begin_file_pos, mem_decl->end_file_pos);
                ...
                for (int64_t j = 0; j < enum_type->enum_type.unique_member_values.count;
                     j++) {
        ...
        case AST_Declaration_Kind::STATIC_ASSERT: {
            AST_Expression **p_cond_expr =
                &declaration->static_assert_decl.cond_expression;
            ...
            if (cond_expr->type->kind != AST_Type_Kind::BOOL) {
                if (is_valid_type_conversion(cond_expr, Builtin::type_bool)) {
                    ...
                    zodiac_report_error(resolver->build_data,
                                        Zodiac_Error_Kind::MISMATCHING_TYPES, cond_expr,
                                        "Argument to  static iassert is not of boolean "
                                        "type, and cannot be converted to boolean type");
                    return false;
                }
            }
...
bool try_resolve_struct_declaration(Resolver *resolver,
                                    AST_Declaration *declaration)
{
    ...
    if (!declaration->type) {
        declaration->type = ast_structure_type_new(
        resolver->allocator, declaration, declaration->structure.member_scope);
    ...
    for (int64_t i = 0; i < declaration->structure.member_declarations.count;
         i++) {

        AST_Declaration **p_member_decl =
            &declaration->structure.member_declarations[i];
    ...
    for (int64_t i = 0; i < declaration->structure.member_declarations.count;
        ...
        if (member_decl->kind == AST_Declaration_Kind::VARIABLE) {
            ...
            array_append(&declaration->type->structure.member_types,
            member_decl->type);
        } else {
    ...
    if (declaration->structure.usings.count) {
        ...
        declaration->structure.imported_members = array_create<AST_Declaration *>(
        resolver->allocator, declaration->structure.usings.count * 2);

        for (int64_t i = 0; i < declaration->structure.usings.count; i++) {
            auto using_ident = declaration->structure.usings[i];

            AST_Declaration *using_member = scope_find_declaration(
                declaration->structure.member_scope, using_ident);
            assert(using_member);
            ...
            for (int64_t j = 0;
                ...
                AST_Declaration *import_link = ast_import_link_declaration_new(
                    resolver->allocator, using_member, imported_decl,
            ...
            for (int64_t j = 0;
                 j < decl_of_used_type->structure.imported_members.count; j++) {
...
bool try_resolve_union_declaration(Resolver *resolver,
                                   AST_Declaration *declaration)
{
    ...
    if (!declaration->type) {
        declaration->type = ast_union_type_new(
        resolver->allocator, declaration, declaration->union_decl.member_scope);
        array_append(&resolver->build_data->type_table, declaration->type);
    } else {
    ...
    for (int64_t i = 0; i < declaration->union_decl.member_declarations.count;
         i++) {
        AST_Declaration **p_member_decl =
        &declaration->union_decl.member_declarations[i];
        auto member_decl = *p_member_decl;
    ...
    for (int64_t i = 0; i < declaration->union_decl.member_declarations.count;
        ...
        array_append(&declaration->type->union_type.member_types,
        member_decl->type);
    }
    ...
    switch (statement->kind) {
        ...
        case AST_Statement_Kind::ASSIGNMENT: {
            ...
            if (ident_expr->type != rhs_expr->type) {
                if (is_valid_type_conversion(rhs_expr, ident_expr->type)) {
                    ...
                    zodiac_report_info(resolver->build_data, rhs_expr, "Given type: '%.*s'",
            ...
            if (operand_expr) {
                ...
                if (operand_expr->type != ret_type) {
                    ...
                    if (is_valid_type_conversion(operand_expr, ret_type)) {
                        ...
                        zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::MISMATCHING_TYPES,
                            operand_expr, "Mismatching type in return statement");

                        auto exp_str = ast_type_to_tstring(ret_type);

                        zodiac_report_info(
                            resolver->build_data, func_ts->function.return_type_spec,
                            "Expected type: '%.*s'", (int)exp_str.length, exp_str.data);

                        auto given_str = ast_type_to_tstring(operand_expr->type);

                        zodiac_report_info(resolver->build_data, operand_expr,
                                           "Given type: '%.*s'", (int)given_str.length,
                                           given_str.data);
                        return false;
                    }
                }
            ...
            if (!(cond_expr->type->kind == AST_Type_Kind::BOOL)) {
                if (is_valid_type_conversion(cond_expr->type, Builtin::type_bool)) {
                    ...
                    zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::MISMATCHING_TYPES,
                            cond_expr,
                            "Conditional expression of while statement is not of boolean type, "
                            "and cannot be converted to boolean type");
                    return false;
                }
            }
            ...
            if (cond_expr->type != Builtin::type_bool) {
                if (is_valid_type_conversion(cond_expr, Builtin::type_bool)) {
                    ...
                    zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::MISMATCHING_TYPES,
                            cond_expr,
                            "Conditional expression of for statement is not of boolean type, "
                            "and cannot be converted to boolean type");
                    return false;
                }
            }
            ...
            if (!(cond_expr->type->kind == AST_Type_Kind::BOOL)) {
                if (is_valid_type_conversion(cond_expr, Builtin::type_bool)) {
                    ...
                    zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::MISMATCHING_TYPES,
                            cond_expr,
                            "Conditional expression of if statement is not of boolean type, "
                            "and cannot be converted to boolean type");
                    return false;
                }
            }
        ...
        case AST_Statement_Kind::SWITCH: {
            ...
            for (int64_t i = 0; i < statement->switch_stmt.cases.count; i++) {
                ...
                if (range_count) {
                    resolver_expand_switch_case_ranges(resolver, statement, switch_case,
                                       range_count, statement->scope);
                }
            }
...
bool try_resolve_expression(Resolver *resolver, AST_Expression *expression)
{
    ...
    switch (expression->kind) {
        ...
        case AST_Expression_Kind::IDENTIFIER: {
            auto decl =
                scope_find_declaration(expression->scope, expression->identifier);

            if (!decl) {
                zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::UNDECLARED_IDENTIFIER,
                            expression->identifier, "Reference to undeclared identifier '%s'",
            ...
            if (!(decl->flags & AST_NODE_FLAG_RESOLVED_ID)) {
                if (decl->kind == AST_Declaration_Kind::FUNCTION) {
                    AST_Declaration *current_function =
                        enclosing_function(resolver, expression);
        ...
        case AST_Expression_Kind::DOT: {
            ...
            if (!(parent_expr->flags & AST_NODE_FLAG_RESOLVED_ID))
                return false;
            ...
            if (parent_decl->kind == AST_Declaration_Kind::IMPORT) {
                ...
                scope_find_declaration(ast_module->module_scope, child_ident);

                if (!child_decl) {
                    zodiac_report_error(
                    resolver->build_data, Zodiac_Error_Kind::UNDECLARED_IDENTIFIER,
                    child_ident, "Reference to undeclared identifier: '%s'",
                ...
                if (parent_type->kind == AST_Type_Kind::ARRAY) {
                    ...
                    if (child_ident->declaration)
                        ...
                        child_decl = scope_find_declaration(enum_scope, child_ident);
                    ...
                    bool found = false;
                    for (int64_t i = 0;
                         i < parent_decl->enum_decl.member_declarations.count; i++) {
                    ...
                    if (!found)
                        assert(false && "Enum member not found");
...
#undef MAYBE_CONVERT
        ...
        case AST_Expression_Kind::UNARY: {
            ...
            switch (expression->unary.op) {
                ...
                case UNOP_NOT: {
                    ...
                    assert(tk == AST_Type_Kind::BOOL || tk == AST_Type_Kind::INTEGER ||
        ...
        case AST_Expression_Kind::CALL: {
            ...
            if (!(recursive || use_fn_ts || (callee_decl->flags & AST_NODE_FLAG_TYPED)))
                assert(false && "Internal compiler error");
            ...
            if (args.count != params.count) {
                zodiac_report_error(
                            resolver->build_data, Zodiac_Error_Kind::MISMATCHING_ARGUMENT_COUNT,
                            expression,
                            "Mismatching argument count, expected %d arguments, got %d",
...
#define _ENSURE_ARGS_ARE_TYPED                                                 \
            ...
            if (name == Builtin::atom_exit) {
                ...
                for (int64_t i = 0; i < args.count; i++) {
                    ...
                    if (args[i]->type->kind == AST_Type_Kind::POINTER) { args[i] = ast_cast_expression_new(
                        resolver->allocator, args[i], Builtin::type_s64, args[i]->scope,
                        args[i]->begin_file_pos, args[i]->end_file_pos);
#ifndef NDEBUG
                        bool cast_res =
#endif
                ...
                if (args[0]->type->kind != AST_Type_Kind::BOOL) {
                    if (is_valid_type_conversion(args[0], Builtin::type_bool)) {
                        ...
                        zodiac_report_error(resolver->build_data,
                                            Zodiac_Error_Kind::MISMATCHING_TYPES, args[0],
                                            "First argument of @assert must be of boolean "
                                            "type, or be able to convert to bool implicitly");
                        return false;
                    }
                }
                ...
                _ENSURE_ARGS_ARE_TYPED
                zodiac_report_error(resolver->build_data,
                  Zodiac_Error_Kind::UNIMPLEMENTED, expression,
                  "Builtin call '%s' is not implemented!", name.data);
                return false;
            }
...
#undef _ENSURE_ARGS_ARE_TYPED
        ...
        case AST_Expression_Kind::ADDROF: {
            ...
            AST_Type *pointer_type = build_data_find_or_create_pointer_type(
        ...
        case AST_Expression_Kind::NULL_LITERAL: {
            ...
            if (expression->infer_type_from) {
                ...
                result_type = build_data_find_or_create_pointer_type(
                    resolver->allocator, resolver->build_data, Builtin::type_void);
            }
...
bool try_resolve_aggregate_dereference(Resolver *resolver,
                                       AST_Type *aggregate_type,
                                       AST_Expression *dot_expr)
{
    ...
    if (!child_decl) {
        assert(aggregate_type->kind == AST_Type_Kind::STRUCTURE);
        zodiac_report_error(resolver->build_data,
                            Zodiac_Error_Kind::UNDECLARED_IDENTIFIER, child_ident,
                            "Undeclared identifier: '%.*s'",
                            (int)child_ident->atom.length, child_ident->atom.data);
    ...
    switch (type_spec->kind) {
        ...
        case AST_Type_Spec_Kind::IDENTIFIER: {
            AST_Declaration *decl =
                scope_find_declaration(type_spec->scope, type_spec->identifier);
            if (!decl) {
                zodiac_report_error(
                                resolver->build_data, Zodiac_Error_Kind::UNDECLARED_IDENTIFIER,
                                type_spec->identifier, "Reference to undeclared identifier '%s'",
        ...
        case AST_Type_Spec_Kind::POINTER: {
            ...
            AST_Type *pointer_type = build_data_find_or_create_pointer_type(
            resolver->allocator, resolver->build_data, base_type_spec->type);
        ...
        case AST_Type_Spec_Kind::FUNCTION: {
            ...
            for (int64_t i = 0; i < type_spec->function.parameter_type_specs.count;
                 i++) {
            ...
            AST_Type *function_type = build_data_find_function_type(
                resolver->build_data, param_types, return_type);

            if (!function_type) {
                function_type =
                ast_function_type_new(resolver->allocator, param_types, return_type);
        ...
        case AST_Type_Spec_Kind::ARRAY: {
            ...
            AST_Type *array_type = build_data_find_array_type(
                resolver->build_data, elem_ts->type, elem_count.integer.s64);

            if (!array_type) {
                array_type = build_data_create_array_type(
                resolver->allocator, resolver->build_data, elem_ts->type,
...
AST_Declaration *resolver_get_declaration(AST_Expression *expr)
{
    ...
    switch (expr->kind) {
        case AST_Expression_Kind::INVALID:
            assert(false);
        case AST_Expression_Kind::IDENTIFIER:
            return expr->identifier->declaration;
        case AST_Expression_Kind::POLY_IDENTIFIER:
            assert(false);
        case AST_Expression_Kind::DOT:
            return expr->dot.child_decl;
        case AST_Expression_Kind::BINARY:
            assert(false);
...
{
    if (expr->expr_flags & AST_EXPR_FLAG_CONST)
        return;
    ...
    switch (expr->kind) {
        ...
        case AST_Expression_Kind::BUILTIN_CALL: {
            ...
            } else if (atom == Builtin::atom_sizeof || atom == Builtin::atom_offsetof) {
                is_const = true;
                             ...
                             AST_Type *target_type)
{
    ...
    if (expr->kind == AST_Expression_Kind::INTEGER_LITERAL ||
        ...
        auto null_expr = ast_null_literal_expression_new(
        resolver->allocator, expr->scope, expr->begin_file_pos,
        expr->end_file_pos);
        null_expr->infer_type_from = expr->type;
        new_expr = ast_binary_expression_new(
        resolver->allocator, BINOP_NEQ, expr, null_expr, expr->scope,
        expr->begin_file_pos, expr->end_file_pos);

        if (!try_resolve_expression(resolver, null_expr))
            assert(false);

    } else {
        new_expr = ast_cast_expression_new(resolver->allocator, expr, target_type,
                       expr->scope, expr->begin_file_pos,
                       expr->end_file_pos);
    }

    if (new_expr) {

        if (!try_resolve_expression(resolver, new_expr))
            assert(false);
...
{
    zodiac_report_error(resolver->build_data,
                        Zodiac_Error_Kind::MISMATCHING_TYPES, arg_expr,
                                          ...
                                          bool is_builtin)
{
    ...
    zodiac_report_error(resolver->build_data,
                        Zodiac_Error_Kind::MISMATCHING_TYPES, arg_expr, err_fmt,
                        index);
...
bool case_range_valid(bool signed_type, Integer_Literal val,
                      Integer_Literal end_val)
{
    if (signed_type)
        return val.s64 < end_val.s64;
    else
        return val.u64 < end_val.u64;
}
                                        ...
                                        Scope *switch_scope)
{
    ...
    while (el.bucket) {
        ...
        if (case_expr->kind == AST_Expression_Kind::RANGE) {
            ...
            Const_Value begin_val = const_interpret_expression(begin_expr);

            Const_Value end_val = const_interpret_expression(end_expr);
            ...
            if (begin_val.type->kind == AST_Type_Kind::INTEGER) {
                if (begin_val.type->integer.sign)
                    signed_type = true;
            } else {
            ...
            begin_fp.file_name = string_append(
                resolver->allocator, string_ref("<expanded from range expression> "),
            ...
            Integer_Literal val;
            if (signed_type)
                val.s64 = begin_val.integer.s64 + 1;
            else
                val.u64 = begin_val.integer.u64 + 1;

            while (case_range_valid(signed_type, val, end_val.integer)) {
                AST_Expression *new_expr = nullptr;

                if (signed_type) {
                    new_expr = ast_integer_literal_expression_new(
                    resolver->allocator, val.s64, switch_scope, begin_fp, end_fp);
                } else {
                    new_expr = ast_integer_literal_expression_new(
                    resolver->allocator, val.u64, switch_scope, begin_fp, end_fp);
                }
                ...
                if (begin_expr->type->kind == AST_Type_Kind::ENUM) {
                    ...
                    auto enum_name = enum_type->enum_type.declaration->identifier->atom;

                    auto parent_ident = ast_identifier_new(
                        resolver->allocator, enum_name, switch_scope, begin_fp, end_fp);

                    auto parent_expr =
                    ast_identifier_expression_new(resolver->allocator, parent_ident,
                                                  switch_scope, begin_fp, end_fp);
                    ...
                    if (member_decl) {
                        ...
                        new_expr = ast_dot_expression_new(
                            resolver->allocator, AST_Dot_Expression_Kind::ENUM_MEMBER,
                            parent_expr, child_ident, switch_scope, begin_fp, end_fp);
    ...
    el = bucket_array_locator_by_index(&switch_case->expressions,
                                       first_range_index);
...
bool resolver_check_switch_completeness(Resolver *resolver,
                                        AST_Statement *ast_stmt)
{
    ...
    if (ast_stmt->switch_stmt.default_case)
        return true;
    ...
    for (int64_t i = 0; i < umvs.count; i++)
        array_append(&unhandled_umvs, umvs[i]);
    ...
    if (unhandled_umvs.count) {
        zodiac_report_error(
                    resolver->build_data, Zodiac_Error_Kind::INCOMPLETE_SWITCH, ast_stmt,
        ...
        for (int64_t i = 0; i < report_count; i++) {
            ...
            zodiac_report_error(
                        resolver->build_data, Zodiac_Error_Kind::INCOMPLETE_SWITCH, ast_stmt,
                        "Unhandled enum value: %s.%s", edecl->identifier->atom.data,
                                    ...
                                    Scope *scope)
{
    ...
    bool is_global =
        scope->kind == Scope_Kind::GLOBAL || scope->kind == Scope_Kind::MODULE;
                                   ...
                                   AST_Type *base_type, Scope *mem_scope)
{
    ...
    AST_Type *enum_type =
    build_data_find_enum_type(resolver->build_data, enum_decl);

    if (!enum_type) {
        enum_type =
        ast_enum_type_new(resolver->allocator, enum_decl, base_type, mem_scope);
...
{
    if (!queue_count(&resolver->resolve_jobs))
        return;
...
bool fatal_error_reported(Resolver *resolver)
{
    ...
    for (int64_t i = 0; i < bd->errors.count; i++) {
        auto kind = bd->errors[i].kind;
        if (kind == Zodiac_Error_Kind::UNIMPLEMENTED) {
            return true;
        }
    }
...
String find_module_path(Resolver *resolver, String module_name)
{
    ...
    for (uint64_t i = 0; i < STATIC_ARRAY_LENGTH(candidate_dirs); i++) {
        ...
        String candidate_file_path =
        string_append(ta, candidate_dirs[i], file_name);
    ...
    if (!found)
        assert(false && "Module not found @TODO");
...
String find_zodiac_root(Allocator *allocator, Build_Data *build_data)
{
    ...
    if (!found)
        assert(false && "@TODO Report error module not found");
