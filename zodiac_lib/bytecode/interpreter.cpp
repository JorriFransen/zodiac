                                  ...
                                  s64 arg_count, s64 result_index)
{
    ...
    if (free_stack_size < fn->required_stack_size) {
        ...
        if (interp->free_stack_blocks) {
            ...
            while (free_block) {
                ...
                if (free_block->cap >= fn->required_stack_size) {
                    ZINFO("Reusing stack block, size: %i", free_block->cap);
        ...
        if (!new_block) {
            ...
            ZINFO("Allocating new stack block, size: %i", new_cap);
...
Interpreter_Stack_Frame interpreter_pop_stack_frame(Interpreter *interp)
{
    ...
    if (stack_count(&interp->frames)) {
        ...
        if (current_block != old_block) {
            ...
            ZINFO("Popped stack block with size: %i", old_block->cap);
        }
    }
...
void interpreter_print_from_memory(Interpreter *interp, u8* mem, Type *type, bool quote_strings/*=false*/)
{
