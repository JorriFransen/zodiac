...
void bytecode_validator_report_error(Bytecode_Validator *validator, Bytecode_Instruction_Handle location, const char *fmt, ...)
{
    ...
    // array_append(&validator->errors, ve);
...
#undef VALIDATE_FLOAT_CMP_BINOP
        ...
        case Bytecode_Opcode::INSERT_VALUE: {
            ...
            if (instruction->a.kind != Bytecode_Register_Kind::INVALID) {
                ...
                if (!(instruction->a.type->flags & AST_TYPE_FLAG_AGGREGATE)) {
            ...
            if (!(dest_type->flags & AST_TYPE_FLAG_AGGREGATE)) {
        ...
        case Bytecode_Opcode::EXTRACT_VALUE: {
            ...
            if (instruction->b.type != Type::s32) {
        ...
        case Bytecode_Opcode::AGG_OFFSET_POINTER: {
            ...
            if (!(aggregate_type->flags & AST_TYPE_FLAG_AGGREGATE)) {
...
Array_Ref<Graph_Node> validator_build_block_graph(Bytecode_Function *func)
{
    ...
    auto nodes = array_create<Graph_Node>(temp_allocator(), func->blocks.count + 1);
    ...
    auto exit_node = array_append(&nodes, exit_node_);
...
{
    String_Builder sb = string_builder_create(allocator);

    string_builder_appendf(&sb, "digraph g { subgraph  cluster_%.*s {\n", (int)func->name.length, func->name.data);
    string_builder_appendf(&sb, "    label = \"%.*s\";\n", (int)func->name.length, func->name.data);
    string_builder_appendf(&sb, "    labelloc = \"t\";\n");
    string_builder_appendf(&sb, "    node [style=filled];\n");

    for (int64_t i = 0; i < nodes.count; i++) {
        auto node = nodes[i];
        if (node.block) {

            string_builder_appendf(&sb, "    %.*s [color=%s];\n",
                   (int)node.block->name.length, node.block->name.data,
                   node.returns ? "green" : "red");

            if (node.a && node.a->block) {
                string_builder_appendf(&sb, "    %.*s -> %.*s;\n",
                       (int)node.block->name.length, node.block->name.data,
            ...
            if (node.a && !node.a->block) {
                string_builder_appendf(&sb, "    %.*s -> exit;\n", (int)node.block->name.length, node.block->name.data);
            }

        } else {
            assert(node.returns);
            string_builder_appendf(&sb, "    exit [color=green];\n");
        }
    }

    string_builder_appendf(&sb, "}}\n");

    auto result = string_builder_to_string(&sb, allocator);
