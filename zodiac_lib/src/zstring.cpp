...
void String::__init__(Allocator * allocator, char *cstr, int64_t len)
{
    assert(len >= 0);
...
void String::__init__(Allocator* allocator, wchar_t* wstr, int64_t _length)
{
    ...
    this->data = alloc_array<char>(allocator, ((int64_t)required_size) + 1);
...
bool string_starts_with(const String_Ref &string, const String_Ref &start)
{
    ...
    for (int64_t i = 0; i < start.length; i++) {
...
int64_t string_to_s64(const String_Ref &string, uint64_t base /*= 10*/)
{
    ...
    for (int64_t i = start_index; i < string.length; i++) {
        result *= base;
        int64_t digit_value = _digit_value(string.data[i]);
        result += digit_value;
    }
